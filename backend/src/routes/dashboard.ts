...
dashboardRouter.get("/summary", async (request, response) => {
  ...
  const [totals, recent] = await Promise.all([
    ...
    prisma.transaction.findMany({ where, include: { category: true }, orderBy: [{ date: "desc" }, { createdAt: "desc" }], take: 5 }),
  ]);
  const income = totals.find((item) => item.type === "INCOME")?._sum.amount?.toNumber() ?? 0;
  const expense = totals.find((item) => item.type === "EXPENSE")?._sum.amount?.toNumber() ?? 0;
  response.json({ month: from.toISOString().slice(0, 7), income, expense, balance: income - expense, transactionCount: totals.reduce((sum, item) => sum + item._count._all, 0), recent });
});
