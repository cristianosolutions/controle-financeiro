...
export function RecurrencesView({ items, categories, accounts, cards, onChanged }: Props) {
  ...
  return <section className="content-section">
    <div className="section-heading"><div><p className="eyebrow">Automação</p><h1>Recorrências</h1><p className="muted">Programe compromissos e receitas que se repetem ao longo do tempo.</p></div>{(items.length > 0 || formOpen) && <button className={formOpen ? "ghost-button compact" : "primary-button compact"} onClick={() => formOpen ? setFormOpen(false) : openForm()}>{formOpen ? "Cancelar" : <><Plus size={18} /> Nova recorrência</>}</button>}</div>
    ...
    {!items.length && !formOpen && <div className="empty-state empty-state-featured"><span className="empty-state-icon"><Repeat2 /></span><p className="eyebrow">Comece por aqui</p><h3>Automatize seus compromissos</h3><p>Cadastre receitas e despesas recorrentes para manter seu planejamento sempre atualizado.</p><button className="primary-button compact" onClick={() => openForm()}><Plus size={18} /> Criar primeira recorrência</button></div>}
  </section>;
}
